angular.module('tus_exitos.services', [])
...
.service('PostService', function ($rootScope, $http, $q, WORDPRESS_API_URL, AuthService, BookMarkService){
  ...
  this.getEmisora = function(emisora) {

    var metadataUrl = 'http://tusexitosfm.com/emisora.html';
    var contentRegex = /<body>(.*)<\/body>/;
    var itunesSearchUrl = 'https://itunes.apple.com/search?term=Daddy';
    ...
    switch (emisora) {
    case '627':
      emisoraLink = 'http://server10.servistreaming.com:2199/rpc/jose/streaminfo.get' + '?callback=JSON_CALLBACK';
      break;
    case 'tropical':
      emisoraLink = WORDPRESS_API_URL + 'get_post/' +
        '?post_id=627' +
        '&insecure=cool' +
        '&callback=JSON_CALLBACK';
      break;
    case 'urbano':
        emisoraLink = 'http://www.tusexitosfm.com/emisora.php';
        break;
    case 'test':
            emisoraLink = WORDPRESS_API_URL + 'get_recent_posts/' +
              '?post_type=emisora' +
              '&insecure=cool' +
              '&callback=JSON_CALLBACK';
        break;
    default:
      confirm('Sorry, no se cual es esa emisora!');
  }
    $http.jsonp(emisoraLink)
    .success(function(data) {
      deferred.resolve(data);
    })
    .error(function(data) {
  ...
  //SLider
  this.getRecentSlides = function(page) {
    var deferred = $q.defer();
...
.service('ChartService', function ($rootScope, $http, $q, WORDPRESS_API_URL, AuthService, BookMarkService){

  this.getChart = function(page) {
    var deferred = $q.defer();

    $http.jsonp(WORDPRESS_API_URL +
      'get_recent_posts/' +
      '?post_type=top10&categories=top40' +
      '&order_by=slug&order=DESC' +
      '&insecure=cool&per_page=2' +
      '&callback=JSON_CALLBACK')
    .success(function(data) {
      deferred.resolve(data);
    })
...
.service('Chart2Service', function ($rootScope, $http, $q, WORDPRESS_API_URL, AuthService, BookMarkService){

  this.getChart2 = function(page) {
    var deferred = $q.defer();

    $http.jsonp(WORDPRESS_API_URL +
      'get_recent_posts/' +
      '?post_type=toptropical&categories=top40' +
      '&order_by=slug&order=DESC' +
      '&insecure=cool&per_page=2' +
      '&callback=JSON_CALLBACK')
    .success(function(data) {
...
.service('Chart3Service', function ($rootScope, $http, $q, WORDPRESS_API_URL, AuthService, BookMarkService){

  this.getChart3 = function(page) {
    var deferred = $q.defer();

    $http.jsonp(WORDPRESS_API_URL +
      'get_recent_posts/' +
      '?post_type=topurbano&categories=top40' +
      '&order_by=slug&order=DESC' +
      '&insecure=cool&per_page=2' +
      '&callback=JSON_CALLBACK')
    .success(function(data) {
