angular.module('tus_exitos.controllers', [])
...
// Top 40
.controller('Top40Ctrl', function($scope) {
...
// Urban
.controller('UrbanCtrl', function($scope) {
...
// Tropical
.controller('TropicalCtrl', function($scope) {
    ...
    $cordovaEmailComposer.isAvailable().then(function() {
      // is available
        $cordovaEmailComposer.open({
          to: 'TUSEXITOSFMPROG@TUSEXITOSFM.COM',
          cc: '',
          subject: 'Contact from TusExitosFM App',
          body: ''
        })
        .then(null, function () {
...
.controller('RateAppCtrl', function($scope) {
  ...
  $scope.rateApp = function(){
    if(ionic.Platform.isIOS()){
      AppRate.preferences.storeAppURL.ios = '1041464185';
...
// Slides
    PostService.getRecentSlides(1)
    .then(function(data){

      $scope.totalPages = data.pages;
      $scope.slides = data.posts;
...
.controller('ChartCtrl', function($scope, $rootScope, $state, $ionicLoading, ChartService) {

  // Charts
      ChartService.getChart(2)
      .then(function(data){

        $scope.totalPages = 2;

        $scope.chart = data.posts;
...
.controller('Chart2Ctrl', function($scope, $rootScope, $state, $ionicLoading, Chart2Service) {

  // Charts
      Chart2Service.getChart2(2)
      .then(function(data){
...
.controller('Chart3Ctrl', function($scope, $rootScope, $state, $ionicLoading, Chart3Service) {

  // Charts
      Chart3Service.getChart3(2)
      .then(function(data){
...
.controller('emisoraCtrl', function($scope, emisora_data, $ionicLoading, PostService, AuthService, $ionicScrollDelegate) {
  $scope.post = emisora_data.data;
  $ionicLoading.hide();
